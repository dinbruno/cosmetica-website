...
const CosmeticaMap = ({ dictionary }: { dictionary: Dictionary }) => {
  ...
  return (
    <section className="w-full relative z-20 py-8 sm:py-12 lg:py-16" id="location">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
        {/* Título da seção - visível apenas no mobile */}
        <div className="mb-6 md:hidden">
          <h2 className="text-2xl font-bold text-gray-800 text-center">{dictionary.find_us}</h2>
          <div className="w-12 h-1 bg-primary mx-auto mt-2"></div>
        </div>

        <div className="relative w-full h-64 sm:h-80 md:h-96 lg:h-[500px]">
          {/* Mapa do Google Maps */}
          <div className="absolute inset-0 w-full h-full z-0 rounded-xl sm:rounded-2xl overflow-hidden shadow-xl">
            <iframe
          ...
          {/* Card de Informações com Glassmorphism - Escondido no mobile */}
          <div className="hidden md:block absolute bottom-4 left-4 right-4 md:bottom-8 md:left-8 md:right-auto md:max-w-md z-20">
            <motion.div
        ...
        {/* Informações abaixo do mapa - Visível apenas no mobile */}
        <div className="mt-6 md:hidden">
          <div className="bg-white rounded-xl p-6 shadow-lg border border-gray-100">
            <h4 className="text-lg font-semibold text-primary mb-4">Cosmética Farmácia de Manipulação</h4>
            ...
            {/* Botão para Google Maps - Mobile */}
            <div className="mt-4">
              <a
                href={`https://www.google.com/maps/search/?api=1&query=${latitude},${longitude}`}
                target="_blank"
                rel="noopener noreferrer"
                className="w-full inline-flex items-center justify-center gap-2 bg-primary text-white px-4 py-3 text-sm font-medium rounded-lg shadow-lg transition-colors duration-300 hover:bg-primary/90"
              >
                {dictionary.view_on_google_maps}
                <span aria-hidden="true">→</span>
              </a>
